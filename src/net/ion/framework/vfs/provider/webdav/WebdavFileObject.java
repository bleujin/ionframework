...
public class WebdavFileObject extends AbstractFileObject implements FileObject {
	...
	private void setDavResource(WebdavResource resource, boolean bCheckExists) throws Exception {
		...
		Debug.line(resource.getGetContentType()) ;
		Debug.line(resource.getGetLastModified()) ;
	...
	protected FileObject[] doListChildrenResolved() throws Exception {
		...
		for (int i = 0; i < children.length; i++) {
			...
			/*
			 * WebdavFileObject fo = (WebdavFileObject) getFileSystem().resolveFile( getName().resolveName( UriParser.encode(dav.getName()), NameScope.CHILD));
			 */
			WebdavFileObject fo = (WebdavFileObject) getFileSystem().resolveFile(getFileSystem().getFileSystemManager().resolveName(getName(), UriParser.encode(dav.getName()), NameScope.CHILD));
	...
	/**
	 * Returns the last modified time of this file. Is only called if {@link #doGetType} does not return {@link FileType#IMAGINARY}.
	 */
