...
public class ImageWaterMarker {
	...
	public ImageWaterMarker(String maskFilePath, int alpha, String position) {
	...
	public int waterMarkTo(String sourceFileName, int minWidth, int minHeight) throws IOException {
		...
		int result = waterMarkTo(sourceFile, new FileOutputStream(tmpfile), minWidth, minHeight);
		if (result == 1) {
			if (tmpfile.exists()) {
	...
	public int waterMarkTo(File sourceFile, OutputStream out, int minWidth, int minHeight) throws IOException {
		...
		try {
			...
			if (maskFile.exists()) {
				Image maskImage = new ImageIcon(maskFilePath).getImage();
				if (maskImage != null) {
					...
					if (imp != null) {
						...
						if ((ip.getWidth() >= minWidth && ip.getHeight() >= minHeight)
								&& (ip.getWidth() >= maskImage.getWidth(null) && ip.getHeight() >= maskImage.getHeight(null))) {
					...
					System.out.println("[" + maskFilePath + "] not supported mask type ");
					result = RESULT_NOT_SUPPORTED_TYPE;
				}
			} else {
				System.out.println("[" + maskFilePath + "] not fount mask file");
