...
/**
 * Writes a JSON (<a href="http://www.ietf.org/rfc/rfc4627.txt">RFC 4627</a>) encoded value to a stream, one token at a time. The stream includes both literal values (strings, numbers, booleans and nulls) as well as the begin and end delimiters of objects and arrays.
 * 
 * <h3>Encoding JSON</h3> To encode your data as JSON, create a new {@code JsonWriter}. Each JSON document must contain one top-level array or object. Call methods on the writer as you walk the structure's contents, nesting arrays and objects as necessary:
 * <ul>
 * <li>To write <strong>arrays</strong>, first call {@link #beginArray()}. Write each of the array's elements with the appropriate {@link #value} methods or by nesting other arrays and objects. Finally close the array using {@link #endArray()}.
 * <li>To write <strong>objects</strong>, first call {@link #beginObject()}. Write each of the object's properties by alternating calls to {@link #name} with the property's value. Write property values with the appropriate {@link #value} method or by nesting other objects or arrays. Finally close the object using {@link #endObject()}.
 * </ul>
 * 
...
public class JsonWriter implements Closeable {
	...
	/**
	 * Configure this writer to relax its syntax rules. By default, this writer only emits well-formed JSON as specified by <a href="http://www.ietf.org/rfc/rfc4627.txt">RFC 4627</a>. Setting the writer to lenient permits the following:
	 * <ul>
	...
	/**
	 * Configure this writer to emit JSON that's safe for direct inclusion in HTML and XML documents. This escapes the HTML characters {@code <}, {@code >}, {@code &} and {@code =} before writing them to the stream. Without this setting, your XML/HTML encoder should replace these characters with the corresponding escape sequences.
	 */
	...
	private void string(String value) throws IOException {
		...
		for (int i = 0, length = value.length(); i < length; i++) {
			 ...
			 * 
			 * We also escape '\u2028' and '\u2029', which JavaScript interprets as newline characters. This prevents eval() from failing with a syntax error. http://code.google.com/p/google-gson/issues/detail?id=341
			 */
			switch (c) {
			case '"':
			case '\\':
	 ...
	 * 
	 * @param root
	 *            true if the value is a new array or object, the two values permitted as top-level elements.
	 */
	...
	public JsonWriter jsonElement(String name, JsonObject json) throws IOException {
		name(name) ;
		beginObject() ;
		for (Entry<String, JsonElement> entry : json.entrySet()) {
			writeJsonElement(json, entry.getKey(), entry.getValue()) ; 
		}
		endObject() ;
		return this ;
	}

	private void writeJsonElement(JsonElement parent, String name, JsonElement json) throws IOException {
		if (json.isJsonPrimitive()){
			if (parent.isJsonObject()) name(name) ;
			final JsonPrimitive preEle = json.getAsJsonPrimitive();
			if (preEle.isBoolean()){
				value(preEle.getAsBoolean()) ;
			} else if (preEle.isNumber()) {
				value(preEle.getAsNumber()) ;
			} else {
				value(preEle.getAsString()) ;
			} 
			...
			for(JsonElement ele : json.getAsJsonArray()){
				writeJsonElement(json, name, ele) ;
			} 
			endArray() ;
