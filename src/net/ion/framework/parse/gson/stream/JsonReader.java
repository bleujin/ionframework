...
package net.ion.framework.parse.gson.stream;
...
import net.ion.framework.parse.gson.internal.JsonReaderInternalAccess;
import net.ion.framework.parse.gson.internal.bind.JsonTreeReader;

/**
 * Reads a JSON (<a href="http://www.ietf.org/rfc/rfc4627.txt">RFC 4627</a>) encoded value as a stream of tokens. This stream includes both literal values (strings, numbers, booleans, and nulls) as well as the begin and end delimiters of objects and arrays. The tokens are traversed in depth-first order, the same order that they appear in the JSON document. Within JSON objects, name/value pairs are
 * represented by a single token.
 * 
 * <h3>Parsing JSON</h3> To create a recursive descent parser for your own JSON streams, first create an entry point method that creates a {@code JsonReader}.
 * 
 * <p>
 * Next, create handler methods for each structure in your JSON text. You'll need a method for each object type and for each array type.
 * <ul>
 * <li>Within <strong>array handling</strong> methods, first call {@link #beginArray} to consume the array's opening bracket. Then create a while loop that accumulates values, terminating when {@link #hasNext} is false. Finally, read the array's closing bracket by calling {@link #endArray}.
 * <li>Within <strong>object handling</strong> methods, first call {@link #beginObject} to consume the object's opening brace. Then create a while loop that assigns values to local variables based on their name. This loop should terminate when {@link #hasNext} is false. Finally, read the object's closing brace by calling {@link #endObject}.
 * </ul>
 * <p>
 ...
 * This reader permits numeric values to be read as strings and string values to be read as numbers. For example, both elements of the JSON array {@code [1, "1"]} may be read using either {@link #nextInt} or {@link #nextString}. This behavior is intended to prevent lossy numeric conversions: double is JavaScript's only numeric type and very large values like {@code 9007199254740993} cannot be
 * represented exactly on that platform. To minimize precision loss, extremely large values should be written and read as strings in JSON.
 * 
 * <a name="nonexecuteprefix"/>
 * <h3>Non-Execute Prefix</h3>
 * Web servers that serve private data using JSON may be vulnerable to <a href="http://en.wikipedia.org/wiki/JSON#Cross-site_request_forgery">Cross-site request forgery</a> attacks. In such an attack, a malicious site gains access to a private JSON file by executing it with an HTML {@code <script>} tag.
 * 
 * <p>
 * Prefixing JSON files with <code>")]}'\n"</code> makes them non-executable by {@code <script>} tags, disarming the attack. Since the prefix is malformed JSON, strict parsing fails when it is encountered. This class permits the non-execute prefix when {@link #setLenient(boolean) lenient parsing} is enabled.
 * 
 * <p>
...
public class JsonReader implements Closeable {
	...
	/*
	 * For the next literal value, we may have the text value, or the position and length in the buffer.
	 */
	...
	/**
	 * Configure this parser to be be liberal in what it accepts. By default, this parser is strict and only accepts JSON as specified by <a href="http://www.ietf.org/rfc/rfc4627.txt">RFC 4627</a>. Setting the parser to lenient causes it to ignore the following syntax errors:
	 * 
	 * <ul>
	 * <li>Streams that start with the <a href="#nonexecuteprefix">non-execute prefix</a>, <code>")]}'\n"</code>.
	 * <li>Streams that include multiple top-level values. With strict parsing, each stream must contain exactly one top-level value.
	 * <li>Top-level values of any type. With strict parsing, the top-level value must be an object or an array.
	...
	public JsonToken peek() throws IOException {
		...
		case EMPTY_DOCUMENT:
			...
			if (!lenient && token != JsonToken.BEGIN_ARRAY && token != JsonToken.BEGIN_OBJECT) {
				throw new IOException("Expected JSON document to start with '[' or '{' but was " + token + " at line " + getLineNumber() + " column " + getColumnNumber());
			}
			return firstToken;
		case EMPTY_ARRAY:
	...
	public void nextNull() throws IOException {
		...
		if (token != JsonToken.NULL) {
			throw new IllegalStateException("Expected null but was " + token + " at line " + getLineNumber() + " column " + getColumnNumber());
		}
	 ...
	 *             if the next literal value cannot be parsed as a double, or is non-finite.
	 */
	public double nextDouble() throws IOException {
		peek();
		if (token != JsonToken.STRING && token != JsonToken.NUMBER) {
			throw new IllegalStateException("Expected a double but was " + token + " at line " + getLineNumber() + " column " + getColumnNumber());
		}

		double result = Double.parseDouble(value);

		if ((result >= 1.0d && value.startsWith("0"))) {
			throw new MalformedJsonException("JSON forbids octal prefixes: " + value + " at line " + getLineNumber() + " column " + getColumnNumber());
		}
		if (!lenient && (Double.isNaN(result) || Double.isInfinite(result))) {
			throw new MalformedJsonException("JSON forbids NaN and infinities: " + value + " at line " + getLineNumber() + " column " + getColumnNumber());
		}
	...
	public long nextLong() throws IOException {
		peek();
		if (token != JsonToken.STRING && token != JsonToken.NUMBER) {
			throw new IllegalStateException("Expected a long but was " + token + " at line " + getLineNumber() + " column " + getColumnNumber());
		}
		...
		try {
			...
			if (result != asDouble) {
				throw new NumberFormatException("Expected a long but was " + value + " at line " + getLineNumber() + " column " + getColumnNumber());
			}
		}
	...
	public int nextInt() throws IOException {
		peek();
		if (token != JsonToken.STRING && token != JsonToken.NUMBER) {
			throw new IllegalStateException("Expected an int but was " + token + " at line " + getLineNumber() + " column " + getColumnNumber());
		}
		...
		try {
			...
			result = (int) asDouble;
			if (result != asDouble) {
				throw new NumberFormatException("Expected an int but was " + value + " at line " + getLineNumber() + " column " + getColumnNumber());
			}
		}
	...
	public JsonObject nextJsonObject() throws IOException{
		return JsonUtil.nextJsonObject(this) ;
	}
	
	public JsonArray nextJsonArray() throws IOException{
		return JsonUtil.nextJsonArray(this) ;
	}
	...
	private JsonToken nextInObject(boolean firstElement) throws IOException {
		/*
		 * Read delimiters. Either a comma/semicolon separating this and the previous name-value pair, or a close brace to denote the end of the object.
		 */
		if (firstElement) {
	...
	private JsonToken objectValue() throws IOException {
		/*
		 * Read the name/value separator. Usually a colon ':'. In lenient mode we also accept an equals sign '=', or an arrow "=>".
		 */
		switch (nextNonWhitespace(true)) {
		case ':':
			break;
	...
	private int nextNonWhitespace(boolean throwOnEof) throws IOException {
		...
		while (true) {
			...
			case '#':
				pos = p;
				/*
				 * Skip a # hash end-of-line comment. The JSON RFC doesn't specify this behaviour, but it's required to parse existing documents. See http://b/2571423.
				 */
				checkLenient();
				skipToEndOfLine();
				p = pos;
	 ...
	 *            true for this method to only set the valuePos and valueLength fields and return a null result. This only works if the literal is short; a string is returned otherwise.
	 */
	@SuppressWarnings("fallthrough")
	private String nextLiteral(boolean assignOffsetsOnly) throws IOException {
		...
		findNonLiteralCharacter: while (true) {
			...
			/*
			 * Attempt to load the entire literal into the buffer at once. If we run out of input, add a non-literal character at the end so that decoding doesn't need to do bounds checks.
			 */
			if (i < buffer.length) {
				if (fillBuffer(i + 1)) {
					continue;
	...
	private JsonToken decodeLiteral() throws IOException {
		...
		} else if (valueLength == 5 && ('f' == buffer[valuePos] || 'F' == buffer[valuePos]) && ('a' == buffer[valuePos + 1] || 'A' == buffer[valuePos + 1]) && ('l' == buffer[valuePos + 2] || 'L' == buffer[valuePos + 2]) && ('s' == buffer[valuePos + 3] || 'S' == buffer[valuePos + 3])
