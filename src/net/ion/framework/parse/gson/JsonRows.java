...
class JsonRows extends WebRowSet implements Rows{
	private static final long serialVersionUID = -8968314868464897515L;
	...
	JsonRows(JsonQueryable query) throws SQLException {
		super();
		this.query = query ;
	}

	
	Rows populate(JsonObject json) throws SQLException {
		return populate(new JsonArray().add(json)) ;
	}
	

	public Rows populate(JsonArray listJson) throws SQLException {
		ResultSetMetaData meta = getMetaData() ;

		for (JsonElement ele : listJson.toArray()) {
			if (! ele.isJsonObject()) throw new IllegalArgumentException("only permitted jsonArray of jsonObject") ;
			JsonObject json = ele.getAsJsonObject() ;
			this.moveToInsertRow();
			for (int i = 1 ; i <= meta.getColumnCount(); i++) {
				this.updateObject(i, JsonUtil.findSimpleObject(json, meta.getColumnLabel(i)));
			}
			this.insertRow();
		}
