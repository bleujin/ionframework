 ...
 * 
 * <h3>Defining a type's JSON form</h3> By default Gson converts application classes to JSON using its built-in type adapters. If Gson's default JSON conversion isn't appropriate for a type, extend this class to customize the conversion. Here's an example of a type adapter for an (X,Y) coordinate point:
 * 
 * <pre>
 * @code
 * 
...
public abstract class TypeAdapter<T> {
	...
	/**
	 * Converts {@code value} to a JSON document and writes it to {@code out}. Unlike Gson's similar {@link Gson#toJson(JsonElement, Appendable) toJson} method, this write is strict. Create a {@link JsonWriter#setLenient(boolean) lenient} {@code JsonWriter} and call {@link #write(net.ion.framework.parse.gson.stream.JsonWriter, Object)} for lenient writing.
	 * 
	 * @param value
	...
	/**
	 * This wrapper method is used to make a type adapter null tolerant. In general, a type adapter is required to handle nulls in write and read methods. Here is how this is typically done:<br>
	 * 
	 * <pre>
	 * @code
	 * 
	...
	/**
	 * Converts {@code value} to a JSON document. Unlike Gson's similar {@link Gson#toJson(Object) toJson} method, this write is strict. Create a {@link JsonWriter#setLenient(boolean) lenient} {@code JsonWriter} and call {@link #write(net.ion.framework.parse.gson.stream.JsonWriter, Object)} for lenient writing.
	 * 
	 * @param value
	 *            the Java object to convert. May be null.
	 */
	...
	/**
	 * Converts the JSON document in {@code in} to a Java object. Unlike Gson's similar {@link Gson#fromJson(java.io.Reader, Class) fromJson} method, this read is strict. Create a {@link JsonReader#setLenient(boolean) lenient} {@code JsonReader} and call {@link #read(JsonReader)} for lenient reading.
	 * 
	 * @return the converted Java object. May be null.
	 */
