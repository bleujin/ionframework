package net.ion.framework.mte.util;
 ...
 * <p>
 * <ul>
 * <li>Solves Demarcation: Where does an embedded language begin and where does it end
 * <ul>
 * <li>Escaping
 * <li>Quotation
...
public final class MiniParser {
	...
	public static MiniParser defaultInstance() {
		return new MiniParser(DEFAULT_ESCAPE_CHAR, DEFAULT_QUOTE_CHAR, false, false, false);
	}

	public static MiniParser trimmedInstance() {
		return new MiniParser(DEFAULT_ESCAPE_CHAR, DEFAULT_QUOTE_CHAR, false, true, false);
	}

	public static MiniParser ignoreCaseInstance() {
		return new MiniParser(DEFAULT_ESCAPE_CHAR, DEFAULT_QUOTE_CHAR, true, false, false);
	}
	...
	public MiniParser(final char escapeChar, final char quoteChar, final boolean ignoreCase, final boolean trim, final boolean rawOutput) {
	...
	public String replace(final String input, final String oldString, final String newString) {
		try {
			...
			for (int index = 0; index < input.length(); index++) {
				if (input.regionMatches(ignoreCase, index, oldString, 0, oldString.length())) {
	...
	public List<String> split(final String input, final char separator, final int maxSegments) {
	...
	public List<String> split(final String input, final String separatorSet, final int maxSegments) {
		return splitInternal(input, false, (char) -1, separatorSet, maxSegments);
	}

	public List<String> splitOnWhitespace(final String input, final int maxSegments) {
	...
	// use
	private List<String> splitInternal(final String input, final boolean splitOnWhitespace, final char separator, final String separatorSet, final int maxSegments) {
		if (input == null) {
			return null;
		}
		try {
			...
			for (int index = 0; index < input.length(); index++) {
				...
				final boolean separates = separatedByWhitespace || (separatorSet != null ? separatorSet.indexOf(c) != -1 : c == separator);
				// in case we are not already in the last segment and there is
				// an
				// unsecaped, unquoted separator, this segment is now done
				if (segments.size() != maxSegments - 1 && separates && !isEscaped()) {
	...
	private int indexOfInternal(final String input, final String substring, boolean last) {
		int resultIndex = -1;
		for (int index = 0; index < input.length(); index++) {
			if (input.regionMatches(ignoreCase, index, substring, 0, substring.length()) && !isEscaped()) {
				resultIndex = index;
				if (!last) {
					break;
				}
	...
	public List<String> scan(final String input, final String splitStart, final String splitEnd) {
		return scan(input, splitStart, splitEnd, false);
	}

	public List<String> greedyScan(final String input, final String splitStart, final String splitEnd) {
		return scan(input, splitStart, splitEnd, true);
	}

	public List<String> scan(final String input, final String splitStart, final String splitEnd, boolean greedy) {
		if (input == null) {
			return null;
		}
		try {
			...
			int lastIndexOfEnd = greedy ? lastIndexOfEnd = lastIndexOf(input, splitEnd) : -1;
			char c;
			int index = 0;
			while (index < input.length()) {
				c = input.charAt(index);
				final boolean greedyCond = !started || !greedy || index == lastIndexOfEnd;
				final String separator = started ? splitEnd : splitStart;
				if (input.regionMatches(ignoreCase, index, separator, 0, separator.length()) && !isEscaped() && greedyCond) {
					finish(segments, buffer);
					buffer = new StringBuilder();
					started = !started;
