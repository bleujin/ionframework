...
public class TemplateContext {
	...
	public AnnotationProcessor<?> resolveAnnotationProcessor(String type) {
		return engine.config().resolveAnnotationProcessor(type);
	}

	public <C> Renderer<C> resolveRendererForClass(Class<C> clazz) {
		return engine.config().resolveRendererForClass(clazz);
	}

	public NamedRenderer resolveNamedRenderer(String rendererName) {
		return engine.config().resolveNamedRenderer(rendererName);
	}

	public Encoder getEncoder() {
		return engine.config().encoder();
	}
